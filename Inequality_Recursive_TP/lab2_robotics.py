...
# Evolution of the TP control errors.
...
def kinematics(d, theta, a, alpha):
    ...
    for i in range(len(d)):
        dh = DH(d[i],theta[i],a[i],alpha[i]) # 1.
        T.append(T[-1] @ dh) # 2. and 3.
    return T
...
def jacobian(T, revolute):
    ...
    for i in range(0,len(T)-1): # 2.
        ...
        if revolute[i]:   
            ...
            J[:,i] = np.concatenate((np.cross(z,o),np.zeros((3,1)) )).reshape((1,6)) #prismatic 
    return J
