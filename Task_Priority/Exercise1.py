...
# Robot definition (3 revolute = FOR REDUNDANCY CHECKING = joint planar manipulator)
...
# Simulation params
dt = 1.0/60.0
Tt = 10 # --------- Lab req = Total simulation time
...
def simulate(t):
    ...
    P =  np.eye(3) - (Jinv @ Jbar)   # SLIDE 5 = Null space projector
    
    y = np.array([np.cos(t), 1.5 * np.sin(t), -1.75 * np.sin(t)]).reshape(3, 1)           # SLIDE 5 =  Arbitrary joint velocity
    
    dq = (Jinv @ (K @ err)) + (P @ y) # SLIDE 5 = ζ = Control signal
