...
def simulate(t):
    ...
    # Update robot
    #Three implementations of the mobile base kinematics integration:
    #a) "move" = First move forward, then rotate.
    #b) "rotate" = First rotate, then move forward.
    #c) "both" = Move forward and rotate at the same time.
    
    robot.update(dq, dt,priority)
    ...
    base_plot.append([eta[0,0], eta[1,0]])
...
ax.set_ylabel("Y[m]")
ax.grid()
basepose_array = np.array([base_plot]).reshape((len(base_plot),2))
ee_array = np.array([ee_plot]).reshape((len(ee_plot),2))
plt.plot(basepose_array[:,0], basepose_array[:,1], label="base position")    
plt.plot(ee_array[:,0], ee_array[:,1], label="end-effector position")    

np.save("base_plot_both.npy",basepose_array)
np.save("ee_plot_both.npy",ee_array)
ax.legend()
